...
Styles: plain, accent-bar, soft-fill, outlined, top-rule, side-rule, raised-card, gradient-card, solid-accent, leaf-corner, header-band, accent-header, tinted.
...
Styles: plain, accent-bar, soft-fill, outlined, top-rule, side-rule, raised-card, gradient-card, solid-accent, leaf-corner, marked, tinted.
...
Styles: plain, accent-bar, soft-fill, outlined, top-rule, side-rule, raised-card, gradient-card, solid-accent, leaf-corner, tinted.
...
Styles: plain, accent-bar, soft-fill, outlined, top-rule, side-rule, raised-card, gradient-card, solid-accent, leaf-corner, banded, header-band, tinted.
...
- **hero** (1): The cover, the section divider, the one number or phrase a page IS — at five times body, medium weight, tight. Three to six words, never a sentence: it holds about half of what display text does. A page with NO title, furniture off, and nothing else beside a line or two.
...
- **lead-and-rest** (col-1, col-2): One thing that LEADS and something beside it — five-eighths and three-eighths. A claim with its figures, a picture with the words about it, a chart with its takeaway. The narrow side holds what a third-of-a-page column holds; put the longer content on the lead.

## Templates, and what each wears

- **Cover** (`cover`) — accent full, surface accent · text·hero, text·md, text·sm. THE TITLE PAGE. The occasion or organisation as the eyebrow; the title as the FIRST TEXT BLOCK — this page has no title field, its title is that block, set at display size and centred; one line saying what the document is; and a byline naming who and when. It ships on the brand surface with a `full` accent and no picture, so the theme's own drawing shows until set_page is given an accentSrc — one call puts a photograph behind the words under a scrim. It is ALSO the closing page: the ask as the title, how to reach you as the byline. For a section break, use Divider.
- **Title and figures** (`title-and-figures`) — stat-row·soft-fill ×3. The page after the title, where three facts frame the whole document — a period, a scale, a duration. The figures are a stat-row, so each is a value with a label and a note. If the opener needs a picture down one side, use Title pane; for the title page itself, use Cover.
